...
namespace ShoppingWebCrawler.Host.PlatformCrawlers.WebPageService
{
    ...
    public class AlimamaWebPageService : BaseWebPageService
    {
        ...
        public class AlimamaMixReuestLoader : BaseBrowserRequestLoader<AlimamaMixReuestLoader>
        {
            ...
            /// <returns></returns>
            public async Task<List<Youhuiquan>> GetCommonCoupon(YouhuiquanFetchWebPageArgument queryParas, string ctoken)
            {
                if (null == queryParas || queryParas.ArgumentsList == null || queryParas.ArgumentsList.Count <= 0)
                {
                    return null;
                }

                //查询出所有的商品的非隐藏-（非特定网站合作的优惠活动）优惠券
                List<Youhuiquan> dataList = new List<Youhuiquan>();
                foreach (var paraItem in queryParas.ArgumentsList)
                {
            ...
            private async Task<string[]> QueryHideQuanActivitysAsync(long sellerId, long itemId)
            {
                ...
                var resp = await clientProxy.PostRequestTransferAsync(queryAddress, PostDataContentType.Form, null, null);
                if (null == resp || resp.Content == null)
                {
                    return null;
                }
